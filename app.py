...
dataforseo_available = (
    "DATAFORSEO_LOGIN" in st.secrets and
    "DATAFORSEO_PASSWORD" in st.secrets
)

if dataforseo_available:
    use_dataforseo = st.sidebar.checkbox(
        "Enrich with DataForSEO",
        value=True,  # Enable by default if credentials exist
    ...
    st.sidebar.info("💡 DataForSEO enrichment unavailable. Add credentials to `.streamlit/secrets.toml`")
    st.sidebar.code('DATAFORSEO_LOGIN = "your-email"\nDATAFORSEO_PASSWORD = "your-password"', language="toml")
    ...
    try:
        ...
        if st.button("🚀 Analyze Striking Distance", type="primary"):
            try:
                with st.spinner("Scraping URLs and analyzing... This may take a while."):
                    ...
                    if use_dataforseo and len(results) > 0:
                        ...
                        try:
                            ...
                            results['enrichment_status'] = 'Not Enriched'
                            ...
                            if enriched_count > 0:
                                # Mark enriched rows
                                results.loc[results['search_volume'] > 0, 'enrichment_status'] = 'Enriched'
                                st.success(f"✅ Enriched {enriched_count}/{len(results)} keywords with DataForSEO data")

                                # Show sample
                                sample = results[results['search_volume'] > 0][['query', 'search_volume', 'keyword_difficulty', 'cpc']].head(3)
                                st.info("Sample enriched keywords:")
                                st.dataframe(sample, use_container_width=True)
                            else:
                                st.warning("⚠️ DataForSEO enrichment enabled but no data retrieved. Check API credits or keyword availability.")
                ...
                # Store results and metadata in session state to persist across reruns
        ...
        # Display results (from session state to persist across reruns)
        if 'analysis_results' in st.session_state:
            ...
            csv = results[export_columns].to_csv(
                index=False,
                decimal='.',  # Force period as decimal separator
                float_format='%.1f'  # Limit floats to 1 decimal place
            )
        ...
        # Retrieve meta_df from session state
        if 'meta_df' not in st.session_state:
            st.warning("⚠️ Meta data not available. Please run analysis first.")
        else:
            ...
            if "OPENROUTER_API_KEY" not in st.secrets:
                st.error("⚠️ OpenRouter API key not configured. Cannot generate optimization plans.")
                st.info("💡 Add your OpenRouter API key to `.streamlit/secrets.toml` to enable AI-powered optimization recommendations.")
                st.code('OPENROUTER_API_KEY = "your-api-key-here"', language="toml")
            else:
                ...
                if selected_urls and st.button("🚀 Generate Optimization Plans", type="primary", key="generate_optimization_button"):
                    with st.spinner("Generating AI-powered recommendations... This may take a few minutes."):
                        try:
                            ...
                            st.info("💡 Check your internet connection and OpenRouter API key.")
                        except KeyError as e:
                            st.error(f"❌ Missing required data field: {str(e)}")
                            st.info("💡 This might indicate a data parsing issue. Please check your input files.")
                        except Exception as e:
